...
def tokenize_commands(command_sequences):
    ...
    for sequence in command_sequences:
        ...
        for command in sequence:
            ...
            # Use literal values for coordinates and velocity
            tokens = [command_type_token, float(x), float(y), float(z), float(velocity)]
...
# Train the model
cnn_lstm_model.fit(X, y_padded, epochs=10, batch_size=2)
